---
title: "GenAI prep: data wrangling with `filter`, `select`, and `arrange`"
author:
  - "Jessica Cooperstone"
date: "2025-10-13"
categories: [r-basics, tidyverse, github-copilot, gen-ai]
...
We are taking this approach so we are not starting using generative AI without any knowledge of our task or functions, so we can better see how its working and how we can make it work for us.
...
We won't be using Github Copilot today but we will use it next week. 

## Why do we wrangle our data?
...
```{r}
# install.packages("tidyverse") # i have version 2.0.0
# install.packages("janitor") # i have version 2.2.1
...
::: {.callout-note}
Note that you can access a single column in a dataframe by using the `$` (dollar sign operator). This will give you your column as a vector.
:::

```{r, eval = FALSE}
# look how annoying it is to refer to a column that has unconventional naming
...
By default, `clean_names()` converts to "snake_case" (where words are separated by an underscore instead of a space).
...
We can remove columns using the `!` operator: you can read the `!` as "not".
...
It's not too surprising that the biggest countries in forest-feasible latitudes have the most forest.
