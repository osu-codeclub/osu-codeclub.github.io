...
and would render making changes to the repeated code problematic as well.
...
We can iterate over our sets with a `for` loop as follows ---
...
```{r}
map_vec(.x = list(set1, set2, set3), .f = function(x) sum(is.na(x)))
```
...
complex situations --- most commonly when we've written our own function.
