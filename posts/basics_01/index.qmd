...
image: img/rstudio-ide-clip.png
---
...
We are going to begin this semester's Code Club right from the beginning in working in R. This session does not assume any prior knowledge of R or other programming languages.
...
But first, what is the difference between R and RStudio? R is a programming language while RStudio has a graphical user interface (GUI) that allows you to more easily use R. You can run R just using the R console (i.e., standalone R) but it is really easier and you will find additional functionality if you use RStudio. We need to download both R and RStudio.
...
![A screenshot of the RStudio IDE](img/rstudio-ide-clip.png)
...
The concept of a directories and your working directory is an important one when coding. The directory of your computer is a system of files with a defined structure. Each file has a specific directory, or path. Directories are hierarchical, and slightly different on different operating systems (e.g., Mac, Windows, Linux). If you open up your Finder (Mac) or File Explorer (Windows), you can navigate around the different locations on your computer.
...
Using an Rproject (`.RProj`) will help you with working directory management. An Rproject allows you to keep all of the files associated with a particular project together - from input data, output data, code, resulting figures etc.
